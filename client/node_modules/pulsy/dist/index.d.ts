...
declare function configurePulsy(config: PulsyConfig): void;
declare function createStore<T>(name: string, initialValue: T, options?: {
    persist?: boolean | PersistenceOptions;
    middleware?: Middleware<T>[];
    memoize?: boolean;
}): void;
declare function getStoreValue<T>(name: string): T | undefined;
declare const createSetter: <T>(name: string) => (newValue: T | ((prevValue: T) => T)) => Promise<void>;
declare function setStoreValue<T>(name: string, value: T): void;
declare function initializePulsy(storeConfigs: Record<string, any>, config?: PulsyConfig): void;
/**
 ...
 * @returns void
 */
declare function clearPersistedStores(storageType?: Storage): void;
/**
 ...
 */
declare function usePulsy<T>(name: string): [T, (newValue: T | ((prevValue: T) => T)) => Promise<void>];
/**
 ...
 */
declare const createNamespacedStore: (namespace: string) => <T>(key: string) => [T, (newValue: T | ((prevValue: T) => T)) => Promise<void>];
/**
 ...
 * @returns {void}
 */
declare function addMiddleware<T>(name: string, middleware: Middleware<T>): void;
/**
 ...
 */
declare function useTimeTravel<T>(name: string): [
    T,
    (newValue: T | ((prevValue: T) => T)) => Promise<void>,
    () => void,
    () => void,
...
declare function createComputedStore<T>(name: string, computeFn: () => T, dependencies: string[]): void;
declare function composeStores<T extends Record<string, any>>(name: string, storeMap: {
    [K in keyof T]: string;
}): [() => T, (updates: Partial<{
    [K in keyof T]: any;
}>) => Promise<void>];
 ...
 * ```
 */
declare function createActions<S, T>(storeName: string, actionHandlers: Record<string, ActionHandler<S, T>>): Record<string, ActionCreator<T>>;
